...
class HistoryFragment : Fragment() {
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                for (orderSnapshot in snapshot.children) {
                    ...
                    if (orderData != null) {
                        ...
                        val arrayIDMaMonAn = orderData["ArrayIDMaMonAn"] as? List<Map<String, Any>>
    ...
    private fun filterOrder(order: DatHang): Boolean {
        ...
        order.listMonAn?.forEach { item ->
            val monAnRef = FirebaseDatabase.getInstance().getReference("MonAn").child(item.maMonAn.toString())
            monAnRef.addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    val gia = snapshot.child("gia").value as? Long ?: 0
                    val trangThaiGiamGia = snapshot.child("trangThaiGiamGia").value as? Long ?: 0
                    val giaChinhThuc = gia * (100 - trangThaiGiamGia) / 100
                    totalPrice += giaChinhThuc * (item.soLuong ?: 1)
                }
            ...
            listMonAn.forEach { item ->
                ...
                monAnRef.addListenerForSingleValueEvent(object : ValueEventListener {
                    override fun onDataChange(snapshot: DataSnapshot) {
                        ...
                        // Tính giá chính thức sau khi giảm giá
                        val giaChinhThuc = gia * (100 - trangThaiGiamGia) / 100
                        val thanhTien = giaChinhThuc * (soLuong ?: 1)
                        totalAmount += thanhTien

                        // Cập nhật chuỗi món ăn
                        itemsStringBuilder.append("- $tenMonAn ($giaChinhThuc VNĐ) - Số lượng: $soLuong\n")
                        ...
                        if (itemsProcessed == listMonAn.size) {
                            ...
                            tvTotalPrice.text = "Tổng tiền: $totalAmount VNĐ"
                        }
                    }
